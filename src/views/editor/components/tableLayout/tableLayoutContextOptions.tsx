...
import {
  ...
  DeleteContextMenu,
  MergeCellsContextMenu,
  SplitCellsContextMenu,
...
export function TableLayoutContextOptions({
  editor,
}: TableContextOptionsProps) {
  const {
    mousePosition,
    theme: { menuLabel },
        ...
        if ($isTableSelection(selection)) {
          ...
          for (const node of selectedNodes) {
            if ($isTableCellNode(node)) {
              anyMergedNode =
                anyMergedNode || node.getColSpan() > 1 || node.getRowSpan() > 1;
            }
          }
  ...
  return (
    <>
      ...
      {insideTableLayout && <DeleteContextMenu />}
