      ...
      () => {
        ...
        if ($isRangeSelection(selection)) {
          ...
          if (!$isTableCellNode(cellNode))
            throw Error("TableContextColumnRemove: expecetd cell node");
          ...
          for (const node of selection.getNodes()) {
            if ($isTableCellNode(node)) {
              if (tableBodyNode == $getTableNodeFromLexicalNodeOrThrow(node)) {
                const testColumnID = $getTableColumnIndexFromTableCellNode(
                  node,
                  resolvedTable,
                );
        ...
        if (!tableNode)
          throw Error("TableContextColumnRemove: null Table Node");
