...
import { FileSystemWorkerImplementation, FileSystemWorkerInterface } from "@/interfaces/system/fileSystem/fileSystemWorkerInterface";
import { notNullOrThrowDev, variableExistsOrThrow } from "@utils";
...
let globalDBX: DropboxAPI.Dropbox | null = null;

export const DropboxFileSystemWorkerImplementation: FileSystemWorkerInterface = {
    ...FileSystemWorkerImplementation,

  registerFileSystemWorker(authOptions: DropboxAPI.DropboxOptions): void {
    globalDBX = new DropboxAPI.Dropbox(authOptions);
  },

  async downloadFileAsync(path: string): Promise<[result: DownloadResult]> {
    notNullOrThrowDev(globalDBX);
    let respond: DropboxFileBlobResponse;
    try {
      respond = await globalDBX.filesDownload({ path: path });
    } catch (error) {
      ...
      throw HandleDownloadError(path, downloadError.error); // promise returns DropboxResponseError<Error<files.DownloadError>> (there is a mistake in index.d.ts)
    }
    ...
    const fileHash: string = await calculateFileHash(file);

    if (fileHash !== fileMeta.content_hash) {
      throw { status: FileSystemStatus.MismatchHash };
    }
...
new WebWorkerThread(DropboxFileSystemWorkerImplementation);
