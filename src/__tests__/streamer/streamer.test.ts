...
async () => {
        test('Upload Multiple Files',
            async () => {
                ...
                for ( let i = 0; i < 8; ++i ) {
                    ...
                    $getFileManager().uploadFile(`/notes/${i}`, new Blob([JSON.stringify(file)]), FileUploadMode.Add);
                }
        ...
        async () => {
            ...
            for ( let i = 0; i < 8; ++i ) {
                const file = structuredClone(fileContent);
                file.root.children[0].children[0].text = i.toString();
                $getFileManager().uploadFile(`/notes/0`, new Blob([JSON.stringify(file)]), FileUploadMode.Add);
            }
        ...
        async () => {
            ...
            for ( let i = 0; i < 8; ++i ) {
                const file = structuredClone(fileContent);
                file.root.children[0].children[0].text = i.toString();
                $getFileManager().uploadFile(`/notes/0`, new Blob([JSON.stringify(file)]), FileUploadMode.Replace);
            }
